...
pub type BravoError {
  ...
  /// replaced with `Empty`.
  ///
  /// This is returned by the `lookup` and `take` functions to represent the
  /// absence of an object with the specified key.
  Empty
  ...
  /// table file. This could mean that the specified decoder types are
  /// incorrect, or the table was not created using Bravo.
  ///
  /// Not to be confused with `FileIsNotTable`.
  DecodeFailure
  ...
  /// access to.
  NoFilePermissions
  /// Thrown when the file at the given filepath does not exist or otherwise
  /// could not be found.
  FileDoesNotExist
  /// Thrown when the filesystem path specified is invalid. Possibly caused by
  /// one of the directories in the filepath not existing.
  InvalidPath
  ...
  InvalidObjectCount
  /// Thrown when `file2tab` is called on a file that is not in a valid format
  /// generated by `tab2file`.
  ///
  /// Not to be confused with `DecodeFailure`.
  FileIsNotTable
  /// Runtime error caught in FFI. This is a fallthrough error type.
  ///
  /// This error should not normally be thrown; if it is, please report the
  /// issue.
  ErlangError(String)
}
